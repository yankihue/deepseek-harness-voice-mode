/*
 * deepseek-voice-mode — CLIENT half (dynamic cordis_define code.client body).
 * Talks to the host orchestrator over same-origin HTTP RPC (/__dsh-voice/rpc/*)
 * and streams mic audio over the /__dsh-voice/ws WebSocket (PROTOCOL §1).
 */
return {
  inject: ['timer'],
  apply(ctx) {
    ...
    var stylesSvc = globalThis.styles;
    if (!React || !stylesSvc) { console.error('[dsvm] missing React/styles builtins'); return; }
